using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class EnemyCollision6 : MonoBehaviour {

	[SerializeField] ParticleSystem effect;
	[SerializeField] GameObject bull;
	[SerializeField] float delayTime = 2f;
	[SerializeField] int scorePoint = 5;
	public AudioSource expFx;

	void Start(){
		expFx = GetComponent<AudioSource>();
	}

	void Explode(){
		effect.Play();
		Destroy(gameObject, delayTime);
	}

	void OnTriggerEnter(Collider bull){
			HealthSystem6.health = HealthSystem6.health - 1;
			if(HealthSystem6.health <= 1){
				HealthSystem6.health = 0;
				if (HealthSystem6.health == 0){
					ScoreBoard.score += scorePoint;
				}
				Explode();
				expFx.Play();
			}
	}
}
